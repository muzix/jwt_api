...
then
  ...
  /usr/bin/ffmpeg -threads 3 -i rtmp://localhost/live/$1 -c:v copy -c:a copy -f flv "rtmp://live.twitch.tv/app/${arr[1]}" >> /root/debug.log &
fi
if [ "${arr[2]}" = "aa" ]
then
  echo "youtube empty" >> /root/debug.log
else
  /usr/bin/ffmpeg -threads 3 -i rtmp://localhost/live/$1 -c:v copy -c:a copy -f flv "rtmp://a.rtmp.youtube.com/live2/${arr[2]}" >> /root/debug.log &
fi
if [ "${arr[3]}" = "aa" ]
then
  echo "facebook empty" >> /root/debug.log
else
  /usr/bin/ffmpeg -threads 3 -i rtmp://localhost/live/$1 -c:v copy -c:a copy -f flv "rtmps://live-api-s.facebook.com:443/rtmp/${arr[3]}" >> /root/debug.log &
fi
wait
